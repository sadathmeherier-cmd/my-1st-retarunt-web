...
function Counter({ value, suffix = '', label }: { value: number; suffix: string; label: string }) {
  ...
  return (
    <div ref={ref} className="text-center">
      <motion.span
        className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-display text-gradient block"
      ...
      <span className="text-xs sm:text-sm text-foreground/50 tracking-widest uppercase mt-2 block">
        {label}
      </span>
    </div>
  );
...
<SectionHeading
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center">
            ...
            >
              <div className="aspect-[3/4] relative overflow-hidden rounded-2xl sm:rounded-3xl">
                ...
                <div className="absolute bottom-6 left-6 right-6 sm:bottom-8 sm:left-8 sm:right-8">
                  <motion.div
                    className="text-3xl sm:text-4xl md:text-5xl font-display text-gradient"
                    initial={{ opacity: 0, y: 20 }}
                    whileInView={{ opacity: 1, y: 0 }}
                    viewport={{ once: true }}
                    transition={{ duration: 0.6, delay: 0.3 }}
                  >
                    Chef James<span className="block text-xl sm:text-2xl text-foreground/40 font-sans font-light mt-2">Culinary Director</span>
                  </motion.div>
                </div>
              </div>
            </motion.div>
            ...
            <div>
              <motion.p
                className="text-base sm:text-lg text-foreground/60 leading-relaxed mb-6 sm:mb-8 font-light"
                initial={{ opacity: 0, y: 30 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
              ...
              <motion.div
                className="space-y-5 mb-8 sm:mb-12"
                initial={{ opacity: 0, y: 30 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
              ...
              >
                {timeline.map((item, i) => (
                  <div key={item.year} className="flex gap-4 sm:gap-6 items-start group">
                    <span className="text-gold text-xs sm:text-sm font-mono font-bold whitespace-nowrap pt-0.5 shrink-0">
                      {item.year}
                    </span>
                    <div className="flex-1 pb-5 border-b border-glass-border">
                      <p className="text-xs sm:text-sm text-foreground/60">{item.event}</p>
                    </div>
                  </div>
                ))}
              </motion.div>
          ...
          <motion.div
            className="grid grid-cols-2 md:grid-cols-4 gap-6 sm:gap-8 lg:gap-12 mt-16 sm:mt-24 lg:mt-32"
            initial={{ opacity: 0, y: 30 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
